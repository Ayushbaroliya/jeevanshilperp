...
// Helper for legacy settings normalization
...
export default function FinanceModule({ onNavigate, userPermissions, lang = 'en', selectedSchool, setSelectedSchool, classes = [] }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {activeTab === 'record' && canRecord && <RecordPayment subOnNavigate={onNavigate} dict={dict} prefilledStudentId={prefilledStudentId} selectedSchool={selectedSchool} />}
      {activeTab === 'adjustments' && canAdjust && <FeeAdjustmentModule selectedSchool={selectedSchool} />}
    ...
    const fetchCloudDues = async () => {
      try {
        ...
        studentSnap.forEach((d) => {
          ...
          const arrears = calculateOpeningArrears(data, Number(data.dueAmount || 0), academicYear); // Assume legacy dueAmount acts as arrears boundary initially if transitioning
...
// ────────────────────────────────────────────────────────────────────────────
function RecordPayment({ subOnNavigate, dict, prefilledStudentId, selectedSchool }) {
  ...
  useEffect(() => {
    getDocs(query(collection(db, "students"), where('schoolId', '==', selectedSchool)))
      .then(snap => setAllStudents(snap.docs.map(d => ({ id: d.id, ...d.data() }))));
  }, [selectedSchool]);
  ...
  // Re-calculate the live ledger for the selected student
  useEffect(() => {
    ...
    fetchLedger();
  }, [selectedStudentId, selectedSchool, selectedStudentObj]);
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      // Extract only the allocations belonging to THIS specific payment
      ...
      await runTransaction(db, async (transaction) => {
        ...
        transaction.set(ledgerRef, {
          ...
          date: now,
          status: 'completed',
          recordedBy: auth.currentUser?.uid || 'accountant',
          schoolId: selectedSchool,
          academicYear: selectedStudentObj?.academicYear || getAcademicYear(),
          allocations: myAllocations // Permanent allocation record!
        });
      ...
      <div className="glass-card" style={{ padding: 24 }}>
        ...
        {currentSummary && (
          <div style={{ padding: 16, background: '#f8fafc', borderRadius: 8, marginBottom: 20 }}>
              ...
              <tbody>
                {currentSummary.ledger.map(c => (
                  <tr key={c.id}>
                    ...
                    <td style={{ color: 'green' }}>{c.allocatedPaid}</td>
                    <td style={{ color: 'orange' }}>{c.allocatedAdjusted}</td>
            ...
            <div style={{ marginTop: 10, fontSize: 16, fontWeight: 'bold', color: 'red' }}>
              Total Outstanding: ₹{currentSummary.totalDue}
            </div>
            {currentSummary.advanceCredit > 0 && (
              <div style={{ color: 'green', fontWeight: 'bold' }}>Advance Credit: ₹{currentSummary.advanceCredit}</div>
            )}
          </div>
        )}
        ...
        {/* FIFO PREVIEW */}
        {previewAllocation && (
          <div style={{ padding: 16, background: '#e0f2fe', borderRadius: 8, marginBottom: 20 }}>
            ...
            <ul style={{ margin: 0, paddingLeft: 20 }}>
              {previewAllocation.allocations.filter(a => a.paymentId === 'preview').map((alloc, i) => (
                <li key={i}>Allocating <strong>₹{alloc.amount}</strong> to {alloc.componentId}</li>
              ))}
            </ul>
  ...
  useEffect(() => {
    const fetchLedger = async () => {
      ...
      try {
        ...
        const res = applyPaymentsAndAdjustments(fullCharges, payments, adjustments);
        setCurrentLedger(res.ledger.filter(c => c.netDue > 0)); // Only show unpaid charges
      } catch (e) { console.error(e); }
    };
    fetchLedger();
  }, [studentId, students, selectedSchool]);
...
console.log('FinanceModule.jsx overwritten successfully.');
