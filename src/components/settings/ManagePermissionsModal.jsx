...
export default function ManagePermissionsModal({ staff, currentUser, onClose, onSave }) {
  ...
  const handleSave = async () => {
    if (staff.uid === currentUser?.id || staff.id === currentUser?.id) {
      alert("You cannot modify your own role or permissions (Self-role escalation blocked).");
      return;
    }

    if (role === 'Administrator' && currentUser?.role !== 'Owner' && currentUser?.role !== 'Director') {
      alert("Only an Owner or Director can grant Administrator privileges.");
      return;
    }
      ...
      <div className="glass-card" style={{ width: '100%', maxWidth: 540, maxHeight: '90vh', overflowY: 'auto', padding: 24, position: 'relative' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <select className="form-input" value={role} onChange={e => handleRoleChange(e.target.value)}>
            ...
            <option value="Peon">Peon (No Default Access)</option>
            <option value="Driver">Driver (No Default Access)</option>
            <option value="Watchman">Watchman (No Default Access)</option>
          </select>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
          ...
          <button 
            ...
            type="button" 
            onClick={handleSave} 
            disabled={isSaving || staff.uid === currentUser?.id || staff.id === currentUser?.id}
          >
