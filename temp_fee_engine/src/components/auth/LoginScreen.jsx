...
import { auth, db, staffAuthEmail, signInWithEmailAndPassword, signOut } from '../../firebase';
...
export default function LoginScreen({ onLoginSuccess }) {
  ...
  const handleOwnerLogin = async (e) => {
    ...
    try {
      ...
      if (credential.user.email === ownerEmail) {
      ...
      setErrorMsg(err?.message === 'OWNER_EMAIL_NOT_CONFIGURED'
        ? 'Owner authentication is not configured. Set VITE_OWNER_EMAIL and create the Firebase Auth owner account.'
        : 'Owner login failed. Check the configured owner credentials.');
    } finally {
      setIsLoggingIn(false);
    }
  };
  ...
  const renderPortalSelection = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      >
        <div style={{ width: 40, height: 40, borderRadius: 10, backgroundColor: 'var(--brand-orange)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Shield size={22} />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 800, fontSize: 14, color: 'var(--brand-orange)' }}>Owner / Super Admin</div>
          <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Group Analytics & Management</div>
        </div>
        <ArrowRight size={18} color="var(--brand-orange)" />
      </button>
      ...
      >
        <div style={{ width: 40, height: 40, borderRadius: 10, backgroundColor: 'var(--brand-green)', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <Globe size={22} />
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 800, fontSize: 14, color: 'var(--brand-green)' }}>Interactive Guided Tour</div>
          <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Learn how to use the software</div>
        </div>
